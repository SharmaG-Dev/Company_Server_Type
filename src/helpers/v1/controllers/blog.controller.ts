...
export const handleGetSubComments = async (req: Request, res: Response) => {
  try {
    ...
    const _GetSubComments = await GetSubComments({
      blogId: blogId,
      commentId: commentId,
    })

    if (!_GetSubComments)
      return res.status(400).json({ message: _GetSubComments })
    res
      .status(200)
      .json({ error: false, message: 'success', data: _GetSubComments })
  } catch (error) {
    res.status(500).json({ error: true, message: error })
  }
}
...
export const handleGetComments = async (req: Request, res: Response) => {
  try {
    ...
    if (!_comments)
      return res.status(400).json({ error: true, message: _comments })
...
export const handleLIke = async (req: customRequest, res: Response) => {
  const { blogId, commentId } = req.query as {
    blogId?: string
    commentId?: string
  }
  const { id } = req.user as User
  try {
    const resposne = await RecordLikes({
      userId: id,
      blogId: blogId,
    ...
    if (!resposne)
      return res.status(400).json({ error: true, message: resposne })
...
export const handleUnlike = async (req: customRequest, res: Response) => {
  const { commentId, blogId } = req.query as {
    commentId?: string
    blogId?: string
  }
  const { id } = req.user as User
  try {
    if (!commentId && !blogId)
      return res
        .status(401)
        .json({ error: true, message: 'provide the commentId or blogId' })
...
export const handleViews = async (req: customRequest, res: Response) => {
  const { profileId, blogId } = req.query as {
    profileId?: string
    blogId?: string
  }
  const { id } = req.user as User
  ...
  try {
    const response = await ViewRegister({
      userid: id,
      profileId: profileId,
      blogId: blogId,
    })
